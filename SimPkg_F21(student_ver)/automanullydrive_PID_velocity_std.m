...
theta=[theta repmat(theta(end),1,10)];
T=[0 dt];
vdata=[];
vk1data=[];
for i=1:100000
...
%     give weights of turning angle reference according to the sparsity of
%     some track coordinate points in front of the car.
    ca=1;
    d2=(cline(:,nearest_idx+1+ca)-cline(:,nearest_idx+ca))'*(cline(:,nearest_idx+1+ca)-cline(:,nearest_idx+ca));
    d=d2^(1/2);
    if d<3
        c1=10; c2=50; c3=2; c0=1;
    else
        c1=3; c2=3; c3=0; c0=1;
    end
...
ylabel('y-m')
yyaxis right
plot(Youtput(:,1),3.6*[0;vdata;vdata(end)])
plot(Youtput(:,1),3.6*[0;vk1data;vk1data(end)])
grid on
hold off
legend('','','Trajectory','Actual velocity','Desired velocity')
