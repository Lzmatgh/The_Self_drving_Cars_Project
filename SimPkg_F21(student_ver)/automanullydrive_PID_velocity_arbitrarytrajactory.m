...
target_path = AvoidObstacles_jjc(TestTrack,Xobs);
target_path=[target_path repmat(target_path(:,end),1,10)];
%% Parameters
Nw=2;
f=0.01;
Iz=2667;
...
cline = target_path(1:2,:); % Center Line
theta = target_path(3,:); % Center Line's Orientation
%%
%plot the track
plot(bl(1,:),bl(2,:),'r')
hold on
...
%     da=(vecnorm(xy-ba(:,nearest_idx)))^(1/2);
    [~, dl] = knnsearch(bl', Youtput(i+1,[1 3]));
    [~, dta] = knnsearch(target_path(1:2,:)', Youtput(i+1,[1 3]));
%     dl=(vecnorm(xy-bl(:,nearest_idx)))^(1/2);
%     dta=(vecnorm(xy-ba(:,nearest_idx)))^(1/2);
    if dl>dta % on the right
        s=1;
    else
        s=-1;
    end
